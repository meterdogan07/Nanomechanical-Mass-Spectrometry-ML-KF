function [confidence, store] = eventDetection_xgb(k, confidence, CONF_THRESH, net, M, d_yk, ix_max, const, store)
    %CONF_THRESH = M/3;
    %net = importKerasNetwork('./ML/model/kalman_tf.h5', 'OutputLayerType','classification');

    store.l = store.l(1:M-1);
    event = pyrunfile("./ML/inference_xgboost.py", "event", x=store.l, M=M);
